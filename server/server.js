...
app.post('/api/add-borrowbook', async (req, res) => {
  ...
  try {
    const book = await booksModel.findOne({bookname});

    if (!book) {
      return res.send({status: 'error', message: 'Book not found'});
    }

    if (book.quantity <= 0) {
      return res.send({status: 'error', message: 'Book out of stock'});
    }
    ...
    // Add the borrowed book to the borrowbooksModel
    await borrowbooksModel.create({
      ...
      publishername,
      publisheddate,
      quantity: 1, // Since the user is borrowing only one book
      daysborrow,
      studentName,
      borrowed: {
        _id: studentid,
...
app.get('/api/getAllBorrowedBooks', async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({status: 'error', error: 'Error fetching borrowed books'});
  }
});

// app.delete('/api/deleteCancelledBook/:bookId', (req, res) => {
//   const bookId = req.params.bookId;
//
//   // Perform the deletion logic here, e.g., remove the book from the database
//
...
app.post('/api/cancel-booking', async (req, res) => {
  ...
  try {
    const bookedItem = await borrowbooksModel.findOne({referenceCode});

    if (!bookedItem) {
      return res.send({status: 'error', message: 'Booking not found'});
    }

    // Update the status to 'canceled' or remove it from the database
    // Here, we will set the status to 'canceled'
    bookedItem.status = 'canceled';
    await bookedItem.save();

    console.log('Booking canceled successfully:', bookedItem);

    res.send({status: 'ok', message: 'Booking canceled successfully'});
  } catch (error) {
    console.error('Error canceling booking:', error);
    res
      .status(500)
      .send({status: 'error', message: 'Error canceling booking', error});
  }
});
...
app.post('/api/author', async (req, res) => {
  try {
    ...
    res.status(500).send({message: 'Error author info', success: false, error});
  }
});
...
app.get('/api/getBorrowedBooksCount', async (req, res) => {
  try {
    // Exclude canceled books from the count
    const borrowedBooksCount = await borrowbooksModel.countDocuments({
      status: {$ne: 'canceled'},
    });
    res.status(200).send({status: 'ok', data: borrowedBooksCount});
  } catch (error) {
    console.log(error);
    res
      .status(500)
      .send({status: 'error', error: 'Error fetching borrowed books count'});
  }
});

app.get('/api/getTotalBooksCount', async (req, res) => {
  try {
    const totalBooksCount = await booksModel.countDocuments({});
    res.status(200).send({status: 'ok', data: totalBooksCount});
  } catch (error) {
    console.log(error);
    res
      .status(500)
      .send({status: 'error', error: 'Error fetching total books count'});
  }
});

app.get('/api/getTotalStudentsCount', async (req, res) => {
  try {
    const totalStudentsCount = await studentModel.countDocuments({});
    res.status(200).send({status: 'ok', data: totalStudentsCount});
  } catch (error) {
    console.log(error);
    res
      .status(500)
      .send({status: 'error', error: 'Error fetching total students count'});
  }
});
...
//     res
//       .status(500)
//       .send({status: 'error', error: 'Error fetching all students'});
//   }
// });
app.get('/api/getAllStudents', async (req, res) => {
  try {
    // Exclude admin users from the list
    const allStudents = await studentModel.find({usertype: {$ne: 'admin'}});
    res.send({status: 'ok', data: allStudents});
  } catch (error) {
    console.log(error);
    res
